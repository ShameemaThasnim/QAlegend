package org.qalegend.constants;

public class Messages {

	public static final String TITTLE_MISMATCH="invalid tittle";
	public static final String EMAIL_ERROR_MISMATCH="invalid email";
	public static final String LOGIN_MISMATCH="invalid Login details";
	public static final String LOGIN_DATE_FAILED="invalid login date";
	public static final String USER_CREATION_FAILED="invalid user";
	public static final String RELOGIN_FAILED="invalid user relogin";
}
